...
import frc.robot.subsystems.vision.Vision.EstimationState;
...
public class LimelightHelpers {
  ...
  public static class PoseEstimate {
    ...
    public Matrix<N3, N1> calculateConfidence(EstimationState state) {

      double singleTagNoisyDistanceMeters = 3.5;
      double singleTagNoiseFactor = 1.5;
      double multiTagNoisyDistanceMeters = 4.5;

      double avgDistance = this.avgTagDist;
      double targetsUsed = this.tagCount;

      double singleTagFactor = 1;

      if (this.tagCount == 1) {
        singleTagFactor =
            Math.max(
                1, Math.max(0, avgDistance - singleTagNoisyDistanceMeters) * singleTagNoiseFactor);
      }
      ...
      double confidenceMultiplier =
          Math.max(
              ...
              (Math.max(
                          1,
                          Math.max(0, avgDistance - multiTagNoisyDistanceMeters)
                              * Constants.VisionConstants.DISTANCE_WEIGHT)
                      * singleTagFactor)
                  / (1 + ((targetsUsed - 1) * Constants.VisionConstants.TAG_PRESENCE_WEIGHT)));

      double autoConfidenceMultiplier =
          Math.max(
              1,
              (Math.max(
                          1,
                          Math.max(0, avgDistance - multiTagNoisyDistanceMeters)
                              * Constants.VisionConstants.AUTO_DISTANCE_WEIGHT)
                      * singleTagFactor)
                  / (1 + ((targetsUsed - 1) * Constants.VisionConstants.AUTO_TAG_PRESENCE_WEIGHT)));

      switch (state) {
        case NONE:
          return Constants.VisionConstants.VISION_MEASUREMENT_STANDARD_DEVIATIONS.times(
              confidenceMultiplier);
        case TELEOP:
          return Constants.VisionConstants.VISION_MEASUREMENT_STANDARD_DEVIATIONS.times(
              confidenceMultiplier);
        case TELEOP_YAW:
          return Constants.VisionConstants.YAW_VISION_MEASUREMENT_CORRECTION_DEVIATIONS.times(
              confidenceMultiplier);
        case AUTO:
          return Constants.VisionConstants.AUTO_VISION_MEASUREMENT_STANDARD_DEVIATIONS.times(
              autoConfidenceMultiplier);
        default:
          return Constants.VisionConstants.VISION_MEASUREMENT_STANDARD_DEVIATIONS.times(
              confidenceMultiplier);
      }
  ...
  public static class Limelight {
    ...
    public void setRobotOrientation(double rotation, double rotationRate) {
      LimelightHelpers.SetRobotOrientation(name, rotation, rotationRate, 0, 0, 0, 0);
    }

    public PoseEstimate getPoseEstimate(boolean useMegaTag2) {
      if (useMegaTag2) {
